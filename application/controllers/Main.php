...
class Main extends CI_Controller
{
	...
	public function bookingssnew()
	{
		...
		$data = array(
			...
			'time' => $this->input->post("time"),
			'idUser' => $this->input->post("idUser"),
			'namecus' => $this->input->post("namecus"),
			'status' => ("กำลังจอง"),
	...
	public function finishchecknew()
	{
		$idDental = $this->input->get('idDental');
		$result['data'] = $this->DT->get_finishcheck($idDental);
		$this->load->view('Finishchecknew', $result);
	}
	...
	public function Cus_booking()
	{
		$date    = $this->input->post('dates', TRUE);
		$result['DTS'] = $this->DT->testdate($date);
		$this->load->view('Book', $result);
	}
	public function testsnew()
	{
		$date    = $this->input->post('dates', TRUE);
		$result['DTS'] = $this->DT->testdatenew($date);
		$this->load->view('Booknew', $result);
	}

	public function Cus_booking2()
	{
		$date    = $this->input->post('dates', TRUE);
		$result['DTS'] = $this->DT->testdate($date);
		$this->load->view('Book1', $result);
	}
	public function testssnew()
	{
		$date    = $this->input->post('dates', TRUE);
		$result['DTS'] = $this->DT->testdatenew($date);
		$this->load->view('Book1new', $result);
	}
	...
	public function backupnew()
	{
		...
		$this->DT->update_recordnew($date, $time, $idDental);
